...
├──────────────────────────────────────────────────────────────────┤
│                    第 2 层：语义理解 (DeepSeek V4-Pro / Kimi K2.7)  │
│   双语翻译 · 封面 OCR · 长文本章节边界识别                         │
├──────────────────────────────────────────────────────────────────┤
│                    第 1 层：快速萃取 (GLM-4.7 / DeepSeek V3)        │
...
| **DeepSeek V4-Pro** | DeepSeek | 超长上下文、深度理解 | 长篇小说章节边界识别、结构化提取 |
| **DeepSeek V3** | DeepSeek | 快速推理、双语能力强 | 双语翻译、常规文本处理 |
...
|---|---|---|---|---|
| 章节识别 (TXT) | GLM-4.7 | DeepSeek V4-Pro | 正则匹配失败时 | ~¥0.005 |
...
| 双语翻译 | DeepSeek V3 | DeepSeek V4-Pro | 按需触发 | ~¥0.5/本 |
| 封面 OCR | Kimi K2.7 | MiniMax M3 | 按需触发 | ~¥0.02/张 |
| 长文本章节分析 | DeepSeek V4-Pro | GLM-5.2 | TXT > 50万字 | ~¥0.08/次 |

### 路由策略设计理念

- **分层降级**：每个任务均有首选和降级模型，API 网关自动检测可用性并切换。
- **成本优先**：能用 GLM-4.7 解决的问题不上 GLM-5.2，能用 DeepSeek V3 的不上 V4-Pro。
- **两阶段管道**：轻量模型做萃取（GLM-4.7 / DeepSeek V3），强推理模型做综合（GLM-5.2 / DeepSeek V4-Pro）。成本节省约 46%，速度提升约 70%。
